...
class PartnerComActivitiesPageYearly extends StatefulWidget {
  @override
  _PartnerComActivitiesPageYearlyState createState() => _PartnerComActivitiesPageYearlyState();
}

class _PartnerComActivitiesPageYearlyState extends State<PartnerComActivitiesPageYearly> {
  dynamic sum = 0;
  var amount = <dynamic>[];
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(child: Scaffold(
        ...
        bottomNavigationBar: TotalAmount(day:'$kYearly ',name: sum <=  0?'0':'#${sum.toString()}',),
                  ...
                  delegate: SliverChildListDelegate([
                    Column(
                      children: <Widget>[
                        ...
                        itemsData.length == 0 && progress == true ? ErrorTitle(errorTitle:'$kNoVendorYear for  ${PageConstants.companyName }'):
                        ListView.builder(
...
.collection("vendorDaily")
        ...
        .now().year).orderBy('dt', descending: true).limit(Variables.yearLimit)
        .get();
...
.collection("vendorDaily")
        .where('cbi', isEqualTo: PageConstants.companyUD)
        .where('yr', isEqualTo: DateTime.now().year).orderBy('dt', descending: true).
