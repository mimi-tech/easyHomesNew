...
class PartnerComActivitiesPageMonthly extends StatefulWidget {
  @override
  _PartnerComActivitiesPageMonthlyState createState() => _PartnerComActivitiesPageMonthlyState();
}

class _PartnerComActivitiesPageMonthlyState extends State<PartnerComActivitiesPageMonthly> {
  dynamic sum = 0;
  var amount = <dynamic>[];
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(child: Scaffold(
        ...
        bottomNavigationBar: TotalAmount(day:'$kMonthly ',name: sum <=  0?'0':'#${sum.toString()}',),
                  ...
                  delegate: SliverChildListDelegate([
                    Column(
                      children: <Widget>[
                        ...
                        itemsData.length == 0 && progress == true ? ErrorTitle(errorTitle:'$kNoVendorMonth for  ${PageConstants.companyName }'):
                        ListView.builder(
...
.collection("vendorDaily")
        ...
        .now().year).orderBy('dt', descending: true).limit(Variables.weeksLimit)
        .get();
...
.collection("vendorDaily")
        .where('cbi', isEqualTo: PageConstants.companyUD)
        .where('mth', isEqualTo: DateTime
        .now().month).where('yr', isEqualTo: DateTime
        .now().year).orderBy('dt', descending: true).
