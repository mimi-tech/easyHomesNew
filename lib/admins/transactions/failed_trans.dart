...
class FailedTrans extends StatefulWidget {
  @override
  _FailedTransState createState() => _FailedTransState();
}

class _FailedTransState extends State<FailedTrans>with TickerProviderStateMixin{
  @override
  void initState() {
  ...
  Future<void> getPaymentList() async {
    ...
    http.Response res = await http.get('https://api.paystack.co/transaction?perpage=${int.parse('50')}&status=failed' as Uri,
  ...
  Future<void> getPaymentListFlutterWave() async {
    http.Response res = await http.get('https://api.flutterwave.com/v3/transactions?from=2020-12-01&to=$date&status=failed' as Uri,
