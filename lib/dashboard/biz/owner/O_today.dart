...
class OToday extends StatefulWidget {
  @override
  _OTodayState createState() => _OTodayState();
}

class _OTodayState extends State<OToday> {
    ...
    return SafeArea(child: Scaffold(
        ...
        body: CustomScrollView(slivers: <Widget>[
          SliverAppBar(
              ...
              flexibleSpace: TxnAmount(title:"Today's earnings".toUpperCase() ,
                amount: DateTime.now().day ==    Constant1.workingDocuments![0]['day']?'${VariablesOne.numberFormat.format(  Constant1.workingDocuments![0]['p3d']).toString()}':'0.00',
                orderCount:  DateTime.now().day ==   Constant1.workingDocuments![0]['day']?  Constant1.workingDocuments![0]['dai'].toString():'0',
              )),
                              ...
                              NewBookingDetails(
                                ...
                                quantity:  itemsData[index]['cQ'],
                                  ...
                                  type: ''.toString(),
                                     ...
                                     itemsData[index]['by'] == true?
                                     AmountOrders( gas: itemsData[index]['amt'],amount: itemsData[index]['p3'],mop: itemsData[index]['mp'] ,)
                                         :
                                    itemsData[index]['acy'] == null? AmountOrders( gas: itemsData[index]['aG'],amount: itemsData[index]['p3'],mop: itemsData[index]['mp'] ,)
                                        :AmountOrderNew(
                                      gas: itemsData[index]['aG'],
                                      cye: itemsData[index]['py3'],
                                      cylinder: itemsData[index]['acy'],
                                      amount: itemsData[index]['p3'],
                                      mop: itemsData[index]['mp'],
                                      total: itemsData[index]['py3'] + itemsData[index]['p3'],
                                    )
...
.collection("vendorDaily").where(
        'od', isEqualTo: Variables.userUid)
      ...
      setState(() {
       ...
       for (DocumentSnapshot document in documents) {
        ...
        setState(() {
          ...
          loadQuantity = List.from(data['cQ']);
             loadKg = List.from(data['kg']);


          print(loadKg);
          print(loadQuantity);
          print(loadQuantity.length);
          print(loadKg.length);
...
.collection("vendorDaily").where(
        ...
        .where('yr',isEqualTo: DateTime.now().year)
        .where('mth',isEqualTo: DateTime.now().month)
        .where('day',isEqualTo: DateTime.now().day).orderBy('dt',descending: true).startAfterDocument(_lastDocument).limit(Variables.limit)
        .get();
