...
class SecondEstimate extends StatefulWidget {
  @override
  _SecondEstimateState createState() => _SecondEstimateState();
}

class _SecondEstimateState extends State<SecondEstimate> {
  Widget spacer() {
...
late  Timer _timer;
  ...
  void initState() {
    ...
    getTotal();
    Variables.cytCounting.forEach((e) => kgSum += e);
    //kgSum = Variables.cytCounting.fold(0, (previous, current) => previous! + current);
            ...
            body: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  spacer(),
                  NewEstimatedPrice(title: '${numberFormat.format(Variables.sumCylinder)}',gas: '${numberFormat.format(Variables.gasEstimatePrice)}',),
                  spacer(),
                  EstimatedPrice(title: Variables.totalGasKG == 0 || Variables.totalGasKG == null?'0.00':'${numberFormat.format(grandTotal)}',price: kEstimatedPrice2,),
                  spacer(),
  ...
  void getTotal() {
   ...
   /* _timer = Timer.periodic( Duration(milliseconds: 500), (timer) {
      if((Variables.totalGasKG == null) || (Variables.totalGasKG == '')){
        setState(() {
          Variables.gasEstimatePrice = kgSum *Variables.cloud!['gas'];

        });
      }else{
        setState(() {
          Variables.gasEstimatePrice = int.parse(Variables.totalGasKG) *Variables.cloud!['gas'];
