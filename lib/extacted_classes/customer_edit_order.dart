...
class CustomerEditOrder extends StatefulWidget {
  @override
  _CustomerEditOrderState createState() => _CustomerEditOrderState();
}

class _CustomerEditOrderState extends State<CustomerEditOrder> {
  Widget spacer() {
...
var expectedWeight = '';
            ...
            child: Column(
              ...
              children: [
               ...
               itemsData[0]['vf'] == true ?Text(''):SizedBtn(nextFunction: (){
                 //check if gas station has release the gas
                if( itemsData[0]['gv'] == true){
                  _verifyOrder();

                }else{
                  VariablesOne.notifyErrorBot(title: 'Please hold on, let the gas station deliver your gas');
                }


                 }, bgColor: kLightBrown, title: 'Confirm delivery')


                   : SizedBtn(nextFunction: (){_updateOrder();}, bgColor: kLightBrown, title: 'Update order'),
                spacer(),
              ],
            ),
          ),
  ...
  Future<void> getDetails() async {
    ...
    final QuerySnapshot result = await FirebaseFirestore.instance
        ...
        .collection('customer').where('cud',isEqualTo: Variables.userUid)
...
if (documents.length == 0) {
  ...
  Future<void> _verifyOrder() async {
      ...
      try {
        ...
        //update customer cb true ie customer has booked gas before
      ...
      try {
      //send the customer booking details to database
        FirebaseFirestore.instance.collection("cachedBookings").doc(
            Variables.userUid).set({
              'cKG': itemsData[0]['cKG'],
              'ca': itemsData[0]['ca'],
              'gk': double.parse(_gasKg.text),
              'aG': itemsData[0]['aG'],
              'cQ': itemsData[0]['cQ'],
        ...
        //take the customer to rate the vendor

       // Navigator.push(context, PageTransition(type: PageTransitionType.fade, child: RateVendor()));
Navigator.pop(context);
        Fluttertoast.showToast(
...
print(count);
    if (_quantity.text.isEmpty) {
      ...
      sendOrder();
//check if gas kg is greater than the  weight of cylinder kg
      /*if(double.parse(_gasKg.text.trim()) > 7.0){
      Fluttertoast.showToast(
          msg: 'gas kg has exceeded',
          toastLength: Toast.LENGTH_LONG,
          timeInSecForIosWeb: 10,
  ...
  void showTotalAmount(total) {
    ...
    showCupertinoModalPopup(
      ...
      ///since the user changed the kg, lets check if the wallet is up to the new amount
          ...
          if(Variables.currentUser[0]['mp'] == kCard){
            ...
            if(Variables.currentUser[0]['mp'] == kPromo){
            ...
            if(Variables.currentUser[0]['mp'] == kWallet){
  ...
  void yesPay(total) {
    ...
    FirebaseFirestore.instance.collection('customer').doc(itemsData[0]['vid']).set({
